...
```mermaid
flowchart LR
  C[Candidate] --> WEB[Interview Web (Browser + WebRTC)]
  INT[Interviewer/Recruiter] --> STUDIO[Studio UI (Question bank + rubric)]

  WEB --> GW[OpenHuman Gateway API (Auth + session)]
  STUDIO --> GW

  GW --> ORCH[Interview Orchestrator (State machine)]
  ORCH --> BUS[Service Bus (events/commands)]
  ORCH --> DB[(PostgreSQL)]
  ORCH --> CACHE[(Redis)]
  ORCH --> BLOB[(Blob Storage (audio/transcripts))]

  ORCH --> STT[Speech-to-Text (Azure Speech)]
  ORCH --> LLM[LLM Runtime (Azure OpenAI)]
  ORCH --> RAG[RAG / Knowledge (Azure AI Search)]

  ORCH --> HUM[HUMINEX API (ATS + onboarding)]
