...
const getDefaultWidgetsByRole = (role: EmployeeRole): PortalWidgetConfig[] => {
  const baseWidgets: PortalWidgetConfig[] = [
    ...
    { id: "announcements", order: 7, size: "medium", visible: true },
    { id: "expense-claims", order: 8, size: "small", visible: true },
  ];
  ...
  const roleSpecificWidgets: Record<EmployeeRole, PortalWidgetConfig[]> = {
    staff: [
      ...
      { id: "announcements", order: 6, size: "medium", visible: true },
    ],
    hr: [
      ...baseWidgets,
    ...
    return [
      {
        ...
        description: "Clean daily workspace for individual contributors",
        icon: "User",
        forRoles: ["staff"],
        widgets: getDefaultWidgetsByRole("staff"),
      },
    ];
  }
