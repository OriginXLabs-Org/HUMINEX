...
const GetQuote = () => {
  ...
  const fetchPricingData = async () => {
    try {
      ...
      const [servicesRes, addonsRes, modifiersRes] = await Promise.allSettled([
        platform.from('service_pricing').select('*').eq('is_active', true),
        platform.from('service_addons').select('*').eq('is_active', true),
        platform.from('pricing_modifiers').select('*').eq('is_active', true),
      ]);
  ...
  const handleContinueToOnboarding = async () => {
    ...
    try {
      // Generate client ID
      const { data: clientIdData, error: clientIdError } = await platform.rpc('generate_client_id');
