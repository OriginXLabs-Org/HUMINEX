...
const AdminAuth = () => {
  ...
  useEffect(() => {
    const checkAdminSession = async () => {
      ...
      if (session?.user) {
        ...
        const { data: roleData } = await platform
          .from('user_roles')
          .select('role')
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // First authenticate
      const { data: authData, error: authError } = await platform.auth.signInWithPassword({
      ...
      const { data: roleData, error: roleError } = await platform
        .from('user_roles')
        .select('role')
          ...
          <a href="/" className="inline-flex flex-col items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center justify-center gap-2 mt-1">
                ...
                <span className="text-[#6B8A8E] text-xs uppercase tracking-wider">HUMINEX</span>
              </div>
            </div>
          </a>
        </div>
            ...
            <>
              <div className="text-center mb-8">
                ...
                <p className="text-[#6B8A8E] text-sm">
                  Authorized HUMINEX personnel only
                </p>
              </div>
