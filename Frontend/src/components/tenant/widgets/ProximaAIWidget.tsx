...
const mockData = {
  ...
  highRiskEmployees: [
    { name: "John Smith", team: "Ops", score: 0.78, action: "Schedule 1-on-1", role: "Junior Developer", email: "john.smith@candidate.dev", experience: "2" },
    { name: "Sarah Lee", team: "Sales", score: 0.72, action: "Review workload", role: "Senior Engineer", email: "sarah.lee@candidate.dev", experience: "6" },
    { name: "Mike Chen", team: "Ops", score: 0.68, action: "Career discussion", role: "Solutions Architect", email: "mike.chen@candidate.dev", experience: "11" },
  ],
  payrollAnomalies: [
...
export const ProximaAIWidget: React.FC = () => {
  ...
  const handleStartInterview = (candidate: typeof mockData.highRiskEmployees[0]) => {
    ...
    navigate(`/tenant/openhuman-studio?${params.toString()}`);
  };
        ...
        <div className="bg-gradient-to-br from-[#F7F9FC] to-[#FEF3F2] rounded-2xl p-5">
          ...
          <div className="space-y-2">
            {mockData.highRiskEmployees.map((emp, i) => (
              <div
                key={emp.name}
                className="p-3 bg-white rounded-xl border border-gray-100 hover:border-[#005EEB]/30 hover:shadow-md transition-all animate-fade-up"
                style={{ animationDelay: `${i * 100}ms` }}
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="text-left">
                    <p className="text-sm font-medium text-[#0F1E3A]">{emp.name}</p>
                    <p className="text-xs text-[#6B7280]">{emp.role} • {emp.team}</p>
                  </div>
                  <div className="text-right">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    size="sm"
                    className="h-7 text-xs bg-[#005EEB] hover:bg-[#004ACC]"
                    onClick={() => handleStartInterview(emp)}
                  >
                    Start Interview
                  </Button>
                </div>
