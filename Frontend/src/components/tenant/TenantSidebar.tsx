...
const primaryNav: NavItem[] = [
  { label: "Dashboard", icon: LayoutDashboard, path: "/tenant/dashboard", moduleId: "dashboard" },
];
...
const coreModules: NavModule[] = [
  {
    ...
    color: "#005EEB",
    items: [
      { label: "Employee Directory", icon: Users, path: "/tenant/workforce", moduleId: "workforce" },
      { label: "Portal Access", icon: Key, path: "/tenant/employees", moduleId: "employees", badge: "New", badgeVariant: "success" },
      { label: "Attendance & Leave", icon: Calendar, path: "/tenant/attendance", moduleId: "attendance" },
      { label: "Documents", icon: FileText, path: "/tenant/documents", moduleId: "documents" },
      { label: "Announcements", icon: MessageSquare, path: "/tenant/announcements", moduleId: "announcements" },
    ],
  },
  {
    ...
    color: "#0FB07A",
    items: [
      { label: "Payroll", icon: DollarSign, path: "/tenant/payroll", moduleId: "payroll", badge: "Processing", badgeVariant: "warning" },
      { label: "Finance & Billing", icon: CreditCard, path: "/tenant/finance", moduleId: "finance" },
      { label: "Insurance & Claims", icon: Shield, path: "/tenant/insurance", moduleId: "insurance" },
    ],
  },
  {
    title: "Talent & Hiring",
    ...
    color: "#00C2FF",
    items: [
      { label: "Recruitment", icon: Briefcase, path: "/tenant/recruitment", moduleId: "recruitment" },
      { label: "OpenHuman", icon: Video, path: "/tenant/openhuman-studio", moduleId: "openhuman-studio", badge: "Interview Studio", badgeVariant: "success" },
      { label: "BGV", icon: UserCheck, path: "/tenant/bgv", moduleId: "bgv", badge: "12", badgeVariant: "warning" },
      { label: "Performance", icon: Target, path: "/tenant/performance", moduleId: "performance" },
    ],
  },
  {
    title: "Operations",
    ...
    color: "#FFB020",
    items: [
      { label: "Tasks & Projects", icon: FolderKanban, path: "/tenant/projects", moduleId: "projects" },
      { label: "EMS / Assets", icon: Building, path: "/tenant/ems", moduleId: "ems" },
      { label: "My Requests", icon: ClipboardList, path: "/tenant/requests", moduleId: "requests" },
      { label: "Notifications", icon: Bell, path: "/tenant/notifications", moduleId: "notifications" },
    ],
  },
  {
    ...
    color: "#E23E57",
    items: [
      { label: "Compliance", icon: FileCheck, path: "/tenant/compliance", moduleId: "compliance" },
      { label: "Risk & Governance", icon: AlertTriangle, path: "/tenant/risk", moduleId: "risk" },
      { label: "Identity & Access", icon: Key, path: "/tenant/identity", moduleId: "identity" },
    ],
  },
  {
    ...
    color: "#8B5CF6",
    items: [
      { label: "Proxima AI", icon: Brain, path: "/tenant/intelligence", moduleId: "intelligence", badge: "Insights", badgeVariant: "success" },
      { label: "Huminex Automations", icon: Zap, path: "/tenant/automations", moduleId: "automations" },
      { label: "Managed Ops", icon: Headphones, path: "/tenant/managed-ops", moduleId: "managed-ops" },
    ],
  },
];
...
const settingsModule: NavModule = {
  ...
  color: "#6B7280",
  items: [
    { label: "Organization", icon: Building, path: "/tenant/settings", moduleId: "settings" },
    { label: "Integrations", icon: Link2, path: "/tenant/settings/integrations", moduleId: "settings-integrations" },
    { label: "API Keys", icon: Key, path: "/tenant/settings/api-keys", moduleId: "settings-api-keys" },
    { label: "Billing & Plans", icon: CreditCard, path: "/tenant/settings/billing", moduleId: "settings-billing" },
    { label: "Data Export", icon: Database, path: "/tenant/settings/export", moduleId: "settings-export" },
    { label: "Custom Domain", icon: Globe, path: "/tenant/settings/domain", moduleId: "settings-domain" },
  ],
};
...
export const TenantSidebar: React.FC = () => {
  ...
  const [accessConfig, setAccessConfig] = useState<TenantAccessConfig>(() => readTenantAccessConfig());
  const [expandedModules, setExpandedModules] = useState<string[]>([]);
  const location = useLocation();
  const canViewPlanControls = role === "super_admin" || role === "admin";
  const canAccess = (moduleId: TenantModuleId) => canAccessTenantModule(role, moduleId, accessConfig);
  ...
  const effectiveSettingsModule: NavModule = useMemo(() => ({
    ...settingsModule,
    items: settingsModule.items.filter((item) => {
      if (item.path === "/tenant/settings/billing" && !canViewPlanControls) return false;
      return canAccess(item.moduleId);
    }),
  }), [canViewPlanControls, role, accessConfig]);
  ...
  const filteredCoreModules = useMemo(
    () =>
      coreModules
        .map((module) => ({
          ...module,
          items: module.items.filter((item) => canAccess(item.moduleId)),
        }))
  ...
  const filteredPrimaryNav = useMemo(
    () => primaryNav.filter((item) => canAccess(item.moduleId)),
    [role, accessConfig]
  );
  ...
  useEffect(() => {
    const allModules = [...coreModules, effectiveSettingsModule];
  ...
  }, [location.pathname, effectiveSettingsModule]);
          ...
          <>
            ...
            {item.badge && (
              <Badge
                variant="outline"
                className={cn(
                  "text-[10px] px-2 py-0.5 h-5 font-semibold border max-w-[104px] truncate",
  ...
  return (
    ...
    >
      ...
      <div className={cn(
        ...
        {!sidebarCollapsed && (
          <div className="flex items-center gap-2.5">
            <img src={huminexLogo} alt="HUMINEX" className="h-8 w-8" />
            <div className="leading-tight">
              <span className="block text-sm font-bold text-[#0F1E3A]">HUMINEX</span>
              <span className="text-[10px] font-semibold text-[#005EEB] uppercase tracking-wider">Portal Navigation</span>
            </div>
          </div>
        )}
        {sidebarCollapsed && <img src={huminexLogo} alt="HUMINEX" className="h-8 w-8 mr-1" />}
        <Button
          variant="ghost"
          size="icon"
      ...
      <nav className="flex-1 overflow-y-auto p-3 space-y-1 scrollbar-thin scrollbar-thumb-gray-200 scrollbar-track-transparent">
        {/* Primary Nav */}
        <div className="mb-4">
          {filteredPrimaryNav.map((item) => renderNavItem(item, sidebarCollapsed))}
        </div>
        ...
        <div className="space-y-1">
          {filteredCoreModules.map(renderModule)}
        </div>
        ...
        {/* Settings */}
        {renderModule(effectiveSettingsModule)}
      </nav>
