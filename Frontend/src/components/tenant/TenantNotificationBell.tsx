...
const mockTenantNotifications: TenantNotification[] = [
  {
    ...
    message: "HUMINEX Admin has enabled workflow automations for your organization.",
...
export const TenantNotificationBell = () => {
  ...
  const markAsRead = async (id: string) => {
    try {
      await platform.from("admin_notifications").update({ is_read: true }).eq("id", id);
  ...
  const markAllAsRead = async () => {
    try {
      ...
      if (unreadIds.length > 0) {
        await platform.from("admin_notifications").update({ is_read: true }).in("id", unreadIds);
      }
