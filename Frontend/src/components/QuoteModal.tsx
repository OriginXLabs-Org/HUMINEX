...
export const QuoteModal = ({ open, onOpenChange }: QuoteModalProps) => {
  ...
  const calculatePrice = () => {
    ...
    if (quoteData.couponCode.toLowerCase() === "huminex10") {
      price *= 0.9;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const discountApplied = quoteData.couponCode.toLowerCase() === "huminex10";
      
      // Generate quote number using database function
      const { data: quoteNumberData, error: quoteNumberError } = await platform
      ...
      const { error: insertError } = await platform
        .from('quotes')
        .insert({
        ...
        {step === 1 && (
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              />
              {quoteData.couponCode.toLowerCase() === "huminex10" && (
