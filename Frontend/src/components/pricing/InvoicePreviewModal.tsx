...
export const InvoicePreviewModal = ({
  ...
  const handleSendEmail = async () => {
    ...
    try {
      const { data, error } = await platform.functions.invoke('send-invoice-email', {
  ...
  const handleDownload = async () => {
    ...
    try {
      const { data, error } = await platform.functions.invoke('generate-invoice-pdf', {
        body: { 
          ...getInvoiceData(), 
  ...
  const handleLoginAndPay = async () => {
    ...
    try {
      ...
      const { data, error } = await platform.functions.invoke('create-razorpay-order', {
        body: {
          amount: totalAmount,
          currency: currency,
          invoiceId,
          ...
          <div className="text-center text-xs text-muted-foreground pt-4 border-t border-border">
            <div className="flex items-center justify-center gap-4 mb-2">
              <span className="flex items-center gap-1">
                <Mail className="w-3 h-3" /> support@gethuminex.com
              </span>
              <span className="flex items-center gap-1">
                <Globe className="w-3 h-3" /> www.gethuminex.com
              </span>
            </div>
            <p>OriginX Labs Pvt. Ltd. • GSTIN: ************ • CIN: **************</p>
