...
const AdminPricingManagement = () => {
  ...
  const fetchAllData = async () => {
    ...
    try {
      const [servicesRes, addonsRes, modifiersRes, couponsRes] = await Promise.all([
        platform.from('service_pricing').select('*').order('service_name'),
        platform.from('service_addons').select('*').order('name'),
        platform.from('pricing_modifiers').select('*').order('modifier_type'),
        platform.from('coupon_codes').select('*').order('created_at', { ascending: false }),
      ]);
  ...
  const handleSaveService = async () => {
    ...
    try {
      const { error } = editService.id
        ? await platform.from('service_pricing').update({
        ...
        : await platform.from('service_pricing').insert({
            service_name: editService.service_name,
  ...
  const handleDeleteService = async (id: string) => {
    ...
    try {
      const { error } = await platform.from('service_pricing').delete().eq('id', id);
      if (error) throw error;
  ...
  const handleSaveAddon = async () => {
    ...
    try {
      const { error } = editAddon.id
        ? await platform.from('service_addons').update({
            name: editAddon.name,
        ...
        : await platform.from('service_addons').insert({
            name: editAddon.name,
            description: editAddon.description,
            price: editAddon.price,
            category: editAddon.category,
  ...
  const handleSaveModifier = async () => {
    ...
    try {
      const { error } = editModifier.id
        ? await platform.from('pricing_modifiers').update({
        ...
        : await platform.from('pricing_modifiers').insert({
            modifier_type: editModifier.modifier_type,
            modifier_key: editModifier.modifier_key,
            multiplier: editModifier.multiplier,
            description: editModifier.description,
  ...
  const handleSaveCoupon = async () => {
    ...
    try {
      const { error } = editCoupon.id
        ? await platform.from('coupon_codes').update({
        ...
        : await platform.from('coupon_codes').insert({
            code: editCoupon.code.toUpperCase(),
            discount_type: editCoupon.discount_type,
            discount_value: editCoupon.discount_value,
            max_uses: editCoupon.max_uses,
