...
const planDistributionData = [
  { name: 'Startup', value: 35, color: 'hsl(var(--muted-foreground))' },
  { name: 'Growth', value: 42, color: 'hsl(262, 83%, 58%)' },
  { name: 'Enterprise', value: 18, color: 'hsl(188, 94%, 43%)' },
  { name: 'Enterprise Plus', value: 5, color: 'hsl(160, 84%, 39%)' },
];
    ...
    const fetchAnalytics = async () => {
      try {
        const [tenantsRes, profilesRes, clickstreamRes] = await Promise.all([
          platform.from('client_tenants').select('*').eq('status', 'active'),
          platform.from('profiles').select('id'),
          platform.from('clickstream_events').select('*').limit(1000),
        ]);
