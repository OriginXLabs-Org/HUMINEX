...
const AdminTenantManagement = () => {
  ...
  const createMutation = useMutation({
    mutationFn: async (tenant: typeof newTenant) => {
      ...
      const { error } = await platform.from('client_tenants').insert({ ...tenant, slug, status: 'active' });
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({ tenantId, status }: { tenantId: string; status: string }) => {
      const { error } = await platform.from('client_tenants').update({ status }).eq('id', tenantId);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (tenantId: string) => {
      const { error } = await platform.from('client_tenants').delete().eq('id', tenantId);
      if (error) throw error;
    },
    onSuccess: () => {
