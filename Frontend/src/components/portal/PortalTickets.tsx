...
import { isDemoPortalUser, getMockTickets, MockTicket } from "@/lib/portalMockData";
...
export const PortalTickets = ({ userId }: PortalTicketsProps) => {
  ...
  const [demoTickets, setDemoTickets] = useState<MockTicket[]>([]);

  useEffect(() => {
    if (isDemoMode) {
      setDemoTickets(getMockTickets());
    }
  }, [isDemoMode]);
  ...
  const tickets = isDemoMode ? demoTickets : liveTickets || [];
  const loadingTickets = isDemoMode ? false : isLoading;
    ...
    mutationFn: async () => {
      ...
      if (isDemoMode) {
        const mockTicket: MockTicket = {
          id: `demo-ticket-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
          ticket_number: ticketNumber,
          subject: newTicket.subject,
          description: newTicket.description,
          priority: newTicket.priority as MockTicket["priority"],
          status: "open",
          created_at: new Date().toISOString(),
        };
        setDemoTickets((prev) => [mockTicket, ...prev]);
        return;
      }

      const { error } = await platform.from("support_tickets").insert({
        user_id: userId,
        ticket_number: ticketNumber,
        subject: newTicket.subject,
        description: newTicket.description,
