...
import { isDemoPortalUser, getMockProjects, getMockInvoices, getMockTickets, getMockMeetings } from "@/lib/portalMockData";
...
export const PortalDashboard = ({ userId, profile, userEmail, employeeVisibility }: PortalDashboardProps) => {
  const { role } = useEmployeeRole();
  const isDemoMode = isDemoPortalUser(userId);
  ...
  const { data: projects } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await platform.from("projects").select("*").eq("user_id", userId).order("created_at", { ascending: false }).limit(5);
      return data || [];
    },
    enabled: !!userId,
  });
  ...
  const { data: invoices } = useQuery({
    ...
    queryFn: async () => {
      if (!userId) return [];
      if (isDemoMode) return getMockInvoices();
      const { data } = await platform.from("invoices").select("*").eq("user_id", userId).limit(5);
      return data || [];
    },
    enabled: !!userId,
  });
  ...
  const { data: tickets } = useQuery({
    ...
    queryFn: async () => {
      if (!userId) return [];
      if (isDemoMode) return getMockTickets();
      const { data } = await platform.from("support_tickets").select("*").eq("user_id", userId).limit(5);
      return data || [];
    },
    enabled: !!userId,
  });
  ...
  const { data: meetings } = useQuery({
    ...
    queryFn: async () => {
      if (!userId) return [];
      if (isDemoMode) return getMockMeetings();
      const { data } = await platform.from("meetings").select("*").eq("user_id", userId).gte("scheduled_at", new Date().toISOString()).order("scheduled_at").limit(3);
      return data || [];
    },
    enabled: !!userId,
  });
  ...
  const renderWidget = (widgetId: string) => {
    switch (widgetId) {
      ...
      case "quick-actions":
        return <QuickActionsWidget role={role} canViewFinance={employeeVisibility.finance} />;
      case "meetings":
  ...
  const simpleViewAllowedWidgets = new Set([
    "quick-stats",
    "attendance",
    "leave-balance",
    "tasks",
  ...
  const renderedWidgets = isSimpleEmployeeView
    ? visibleWidgets.filter((w) => simpleViewAllowedWidgets.has(w.id))
    : visibleWidgets;

  const visibilityFilteredWidgets = renderedWidgets.filter((widget) => {
    if (widget.id === "invoices") return employeeVisibility.finance;
    if (widget.id === "payslip") return employeeVisibility.payslips;
    if (widget.id === "expense-claims") return employeeVisibility.benefits;
    return true;
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-heading font-bold text-foreground">
            {isSimpleEmployeeView ? `Welcome, ${profile?.full_name || "Employee"}` : "Welcome back! 👋"}
          </h1>
          <p className="text-muted-foreground">
            {isSimpleEmployeeView
              ? `${profile?.designation || "Senior Engineer"} • ${profile?.department || "Engineering"}`
              : "Here's what's happening today."}
            <Badge variant="secondary" className="ml-2 text-xs capitalize">{role}</Badge>
          </p>
        </div>
        {!isSimpleEmployeeView && (
      ...
      {!isSimpleEmployeeView && <RestrictedWidgetsIndicator restrictedWidgetIds={restrictedWidgets} />}

      <EmployeeHierarchyTree role={role} profileEmail={profile?.email} userEmail={userEmail} />

      {/* Edit Mode Bar */}
      {isEditMode && !isSimpleEmployeeView && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {visibilityFilteredWidgets.map((widget) => (
          <DraggablePortalWidget
            key={widget.id}
            widget={widget}
            isEditMode={isEditMode && !isSimpleEmployeeView}
...
// Sub-components
const QuickStatsWidget = ({ role, projects, pendingAmount, openTickets, meetingsCount }: any) => {
  const stats = role === "staff"
    ? [
        { label: "My Active Projects", value: projects?.filter((p: any) => p.status === "active").length || 0, icon: FolderKanban, color: "from-blue-500 to-cyan-500" },
        { label: "Upcoming Meetings", value: meetingsCount, icon: Calendar, color: "from-emerald-400 to-green-500" },
        { label: "Leave Balance", value: "14 Days", icon: Clock, color: "from-violet-500 to-purple-500" },
        { label: "Payslip Status", value: "Ready", icon: Receipt, color: "from-sky-500 to-indigo-500" },
      ]
    : [
...
const QuickActionsWidget = ({ role, canViewFinance }: { role: string; canViewFinance: boolean }) => (
  <Card className="h-full">
    <CardHeader className="pb-3">
    ...
    <CardContent>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          ...(role !== "staff" ? [{ name: "Create Ticket", icon: HeadphonesIcon, href: "/portal/tickets", color: "bg-purple-500" }] : []),
          ...(canViewFinance ? [{ name: "View Finance & Payslips", icon: Receipt, href: "/portal/invoices", color: "bg-emerald-500" }] : []),
        ].map((action: any) => {
